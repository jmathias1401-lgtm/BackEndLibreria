...
@Repository
@AllArgsConstructor
public class CargoRepository extends BaseRepository {
    private EntityManager entityManager;

    public Object list(CargoListRequest params, String mode) {
        Object response;
        CriteriaQuery query;
        List<Predicate> predicates = new ArrayList<>();
        Map mapParam = Util.dtoTomap(params);
        CriteriaBuilder builder = entityManager.getCriteriaBuilder();
        query = mode.equals("L") ? builder.createQuery(Cargo.class) : builder.createQuery(Long.class);
        ...
        if (params.getSearch() != null) {
            Predicate search = builder.like(root.get("nombrecargo"),"%" + params.getSearch().toUpperCase() + "%");
