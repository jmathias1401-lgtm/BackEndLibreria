...
public class compraRepository extends BaseRepository {
    ...
    public Object list(CompraListRequest params, String mode) {
        Object response;
        CriteriaQuery query;
        List<Predicate> predicates = new ArrayList<>();
        Map mapParam = Util.dtoTomap(params);
        CriteriaBuilder builder = entityManager.getCriteriaBuilder();
        query = mode.equals("L") ? builder.createQuery(Compra.class) : builder.createQuery(Long.class);
        Root root = query.from(Compra.class);
        query.orderBy(builder.desc(root.get("idcompra")));
        ...
        if (params.getSearch() != null) {
            Predicate search = builder.like(
                    builder.upper(builder.concat(builder.concat(root.get("serie"), root.get("correlativo")),
                            builder.concat(builder.concat(root.get("proveedor").get("nombre"), root.get("empleado").get("persona").get("nombre")),
                                    builder.concat(root.get("tipoComprobante").get("descripcion"), "")))),
