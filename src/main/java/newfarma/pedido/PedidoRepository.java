...
public class PedidoRepository extends BaseRepository {
    ...
    public Object list(PedidoListRequest params, String mode) {
        Object response;
        CriteriaQuery query;
        List<Predicate> predicates = new ArrayList<>();
        Map mapParam = Util.dtoTomap(params);
        CriteriaBuilder builder = entityManager.getCriteriaBuilder();
        query = mode.equals("L") ? builder.createQuery(Pedido.class) : builder.createQuery(Long.class);
        ...
        List<String> eqFields = new ArrayList<String>() {{add("idpedido");add("codigopedido");}};
        List<String> likeFields = new ArrayList<String>() {{add("idpedido");add("codigopedido");}};
        criteriaParams = this.addCriterias(criteriaParams, builder, root, mapParam, eqFields, "eq");
        // WITH THAT IS THE LIKE
        if (params.getSearch() != null) {
            Predicate search = builder.like(root.get("idpedido").as(String.class),"%" + params.getSearch().toUpperCase() + "%");
